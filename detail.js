...
detailsForm.addEventListener('submit', (e) => {
  ...
  if (email !== confirmEmail) {
    showError('Email and Confirm Email must match.');
  }

  if (!creditCard) {
    showError('Please enter your Credit Card Number.');
  } else if (!creditCardPattern.test(creditCard)) {
    showError('Credit Card Number should contain exactly 16 digits.');
  }

  if (!expiryDate) {
    showError('Please select the Expiry Date from the calendar.');
  }

  if (!cvv) {
    showError('Please enter the CVC/CVV.');
  } else if (!cvvPattern.test(cvv)) {
    showError('CVC/CVV should contain exactly 3 digits.');
  }
  ...
  if (!errorContainer.innerHTML) {
    // Form is valid, save the values to local storage
    ...
    // Form submitted successfully, you can perform further actions if needed
...
function displayLocalStorageValues() {
    ...
    const nameOnCard = localStorage.getItem('nameOnCard') || '';

    document.getElementById('fullName').textContent = fullName;
    document.getElementById('mobile').textContent = mobile;
    document.getElementById('email').textContent = email;
    document.getElementById('confirmEmail').textContent = confirmEmail;
    document.getElementById('creditCard').textContent = creditCard;
    document.getElementById('expiryDate').textContent = expiryDate;
    document.getElementById('cvv').textContent = cvv;
    document.getElementById('nameOnCard').textContent = nameOnCard;
  }
